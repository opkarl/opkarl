import { useEffect, useRef, useState } from 'react'
import styles from './Hero.module.css'

function getRaceCountdown() {
  const now = new Date()
  let raceDate = new Date(now.getFullYear(), 5, 28) // June 28
  if (now > raceDate) raceDate = new Date(now.getFullYear() + 1, 5, 28)
  const diff = Math.ceil((raceDate - now) / (1000 * 60 * 60 * 24))
  return diff
}

const TICKER_ITEMS = [
  { text: 'Oslo, Norway' },
  { text: 'BSc Computer Technology @ UiB' },
  { text: null, key: 'race' },
  { text: 'Currently optimizing: everything' },
]

export default function Hero() {
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768
  const cyclistRef = useRef(null)
  const terrainRef = useRef(null)
  const rafRef = useRef(null)
  const [tickerVisible, setTickerVisible] = useState([false, false, false, false])
  const daysLeft = getRaceCountdown()

  useEffect(() => {
    const timers = TICKER_ITEMS.map((_, i) =>
      setTimeout(() => {
        setTickerVisible(prev => {
          const next = [...prev]
          next[i] = true
          return next
        })
      }, 600 + i * 100)
    )
    return () => timers.forEach(clearTimeout)
  }, [])

  useEffect(() => {
    if (isMobile) return

    const onScroll = () => {
      if (rafRef.current) cancelAnimationFrame(rafRef.current)
      rafRef.current = requestAnimationFrame(() => {
        const scrollY = window.scrollY
        if (cyclistRef.current) {
          cyclistRef.current.style.transform = `translateX(${scrollY * 0.18}px)`
        }
        if (terrainRef.current) {
          terrainRef.current.style.transform = `translateX(${scrollY * 0.07}px)`
        }
      })
    }

    window.addEventListener('scroll', onScroll, { passive: true })
    return () => {
      window.removeEventListener('scroll', onScroll)
      if (rafRef.current) cancelAnimationFrame(rafRef.current)
    }
  }, [isMobile])

  return (
    <section className={styles.hero} aria-label="Introduction">
      <div className={styles.left}>
        <h1 className={styles.heading}>
          Building clean software.<br />Training for a triathlon.
        </h1>
        <p className={styles.subheading}>
          IT student, teaching assistant, Royal Norwegian Air Force veteran. Based in Oslo.
        </p>

        <div className={styles.ctas}>
          <a
            href="#projects"
            className={styles.btnFilled}
            aria-label="See my work"
            onClick={e => { e.preventDefault(); document.getElementById('projects')?.scrollIntoView({ behavior: 'smooth' }) }}
          >
            See my work
          </a>
          <a
            href={`${import.meta.env.BASE_URL}cv.pdf`}
            className={styles.btnOutline}
            aria-label="Download CV"
            download
          >
            Download CV
          </a>
        </div>

        <div className={styles.ticker} aria-label="Quick facts">
          {TICKER_ITEMS.map((item, i) => (
            <div
              key={i}
              className={`${styles.tickerItem} ${tickerVisible[i] ? styles.visible : ''}`}
              style={{ animationDelay: `${i * 0.08}s` }}
            >
              <span className={styles.dot} aria-hidden="true" />
              {item.key === 'race' ? (
                <span>
                  Race day: <span className={styles.countdown}>{daysLeft} days</span> until June 28
                </span>
              ) : (
                <span>{item.text}</span>
              )}
            </div>
          ))}
        </div>
      </div>

      <div className={styles.right} aria-hidden="true">
        <div className={styles.parallaxContainer}>
          <div ref={terrainRef} className={styles.terrain}>
            <TerrainSVG />
          </div>
          <div ref={cyclistRef} className={styles.cyclist}>
            <CyclistSVG />
          </div>
        </div>
      </div>
    </section>
  )
}

function TerrainSVG() {
  return (
    <svg viewBox="0 0 900 300" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMax meet">
      {/* Far mountains */}
      <polygon points="0,300 120,100 220,180 340,60 460,160 580,80 700,140 820,50 900,120 900,300" fill="#181818" />
      {/* Mid hills */}
      <polygon points="0,300 80,220 200,170 350,200 500,160 650,190 800,155 900,180 900,300" fill="#1a1a1a" />
      {/* Ground */}
      <rect x="0" y="270" width="900" height="30" fill="#1e1e1e" />
      {/* Road */}
      <rect x="0" y="272" width="900" height="6" fill="#252525" />
    </svg>
  )
}

function CyclistSVG() {
  return (
    <svg viewBox="0 0 200 160" xmlns="http://www.w3.org/2000/svg" fill="none">
      {/* Rear wheel */}
      <circle cx="42" cy="118" r="34" stroke="#1d9e75" strokeWidth="3" fill="none" />
      <circle cx="42" cy="118" r="4" fill="#1d9e75" />
      {/* Front wheel */}
      <circle cx="158" cy="118" r="34" stroke="#1d9e75" strokeWidth="3" fill="none" />
      <circle cx="158" cy="118" r="4" fill="#1d9e75" />
      {/* Frame */}
      <line x1="42" y1="118" x2="100" y2="70" stroke="#f0f0f0" strokeWidth="2.5" />
      <line x1="42" y1="118" x2="100" y2="118" stroke="#f0f0f0" strokeWidth="2.5" />
      <line x1="100" y1="118" x2="158" y2="118" stroke="#f0f0f0" strokeWidth="2.5" />
      <line x1="100" y1="70" x2="158" y2="118" stroke="#f0f0f0" strokeWidth="2.5" />
      <line x1="100" y1="70" x2="100" y2="118" stroke="#f0f0f0" strokeWidth="2" />
      {/* Seat post */}
      <line x1="90" y1="70" x2="85" y2="50" stroke="#f0f0f0" strokeWidth="2.5" />
      <line x1="80" y1="48" x2="94" y2="48" stroke="#f0f0f0" strokeWidth="3" />
      {/* Handlebar */}
      <line x1="152" y1="82" x2="155" y2="65" stroke="#f0f0f0" strokeWidth="2.5" />
      <line x1="150" y1="64" x2="162" y2="64" stroke="#f0f0f0" strokeWidth="2.5" />
      {/* Rider torso (aggressive forward lean) */}
      <line x1="87" y1="49" x2="148" y2="68" stroke="#e0e0e0" strokeWidth="3" />
      {/* Head */}
      <circle cx="155" cy="60" r="9" fill="#e0e0e0" />
      {/* Arms */}
      <line x1="140" y1="64" x2="157" y2="64" stroke="#e0e0e0" strokeWidth="2.5" />
      {/* Left leg (down) */}
      <line x1="100" y1="118" x2="110" y2="135" stroke="#e0e0e0" strokeWidth="3" />
      <line x1="110" y1="135" x2="102" y2="148" stroke="#e0e0e0" strokeWidth="3" />
      {/* Right leg (up) */}
      <line x1="100" y1="118" x2="88" y2="105" stroke="#e0e0e0" strokeWidth="3" />
      <line x1="88" y1="105" x2="96" y2="94" stroke="#e0e0e0" strokeWidth="3" />
      {/* Crank */}
      <circle cx="100" cy="118" r="6" fill="none" stroke="#1d9e75" strokeWidth="2" />
    </svg>
  )
}
